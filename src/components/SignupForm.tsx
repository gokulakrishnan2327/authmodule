...
// Define the Redux state shape (you might want to import this from a separate types file)
interface RootState {
  auth: {
    loading: boolean;
...
const SignupForm: React.FC = () => {
  ...
  const { loading, isAuthenticated, profileStatus, error } = useSelector((state: RootState) => state.auth);
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Create the data to submit with the proper structure
    const dataToSubmit = {
      ...signupData,
    ...
    console.log('Submitting signup data:', dataToSubmit);
    
    dispatch(signupUser(dataToSubmit) as any)
      .unwrap()
      .then(() => {
  ...
  const renderErrorTooltip = (fieldName: string): React.ReactNode => {
