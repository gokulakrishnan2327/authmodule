...
const OnboardingStepper: React.FC<OnboardingStepperProps> = ({ onComplete }) => {
  ...
  const { loading, error, successMessage, profileStatus } = useSelector((state: RootState) => state.auth);
      ...
      case 1:
        return (
          <RoleSelection 
            ...
            onChange={(role: string) => updateFormData('role', role)}
          />
        );
      case 2:
        return (
          <InterestsSelection
            ...
            onChange={(interests: string[]) => updateFormData('interests', interests)}
          />
        );
      case 3:
        return (
          <ProfilePhotos
            ...
            onChange={(photo: File | null) => updateFormData('profilePhoto', photo)}
          />
        );
      case 4:
        return (
          <GoalsSelection
            ...
            onChange={(goals: string[]) => updateFormData('goals', goals)}
          />
        );
      case 5:
        return (
          <ContactInfo
            mobileNumber={formData.mobileNumber}
            onChange={(number: string) => updateFormData('mobileNumber', number)}
          />
        );
      default:
        return null;
